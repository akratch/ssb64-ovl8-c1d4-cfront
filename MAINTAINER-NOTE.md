...
`func_ovl8_8037C1D4` now has a zero-difference C++ match.

The matching source is valid C++ and exports the expected mangled symbol. It
passes through SGI's legacy cfront 3.0.1 translator, then through the project's
IDO 7.1 C backend. The resulting function is exactly 60 instructions / `0xF0`
bytes, with zero opcode, register, schedule, or raw-word differences.

This supports a third split at C1D4:

- retain the first IRIX4 region;
- retain the established IDO 7.1 NCC C++ region;
- compile C1D4 alone as C++ through the legacy `-use_cfront` path.
...
complete 7.1 OCC/cfront package would be the ideal final confirmation, but the
compiler mechanism and matching object are fully preserved here.
